(() => {
  ...
  if (!window.BABYLON || !BABYLON.Engine.isSupported()) { ui.loading.textContent='WebGL/Babylon.js در این سیستم قابل اجرا نیست.'; return; }

  const engine = new BABYLON.Engine(ui.canvas,true,{preserveDrawingBuffer:true,stencil:true,antialias:true});
  const scene = new BABYLON.Scene(engine);
  scene.clearColor=new BABYLON.Color4(.72,.86,.92,1); scene.fogMode=BABYLON.Scene.FOGMODE_LINEAR; scene.fogStart=150; scene.fogEnd=420; scene.fogColor=new BABYLON.Color3(.72,.86,.92);
  const camera=new BABYLON.FreeCamera('followCam',new BABYLON.Vector3(0,9,-14),scene); camera.minZ=.1; camera.fov=.88; camera.inputs.clear();
  const hemi=new BABYLON.HemisphericLight('hemi',new BABYLON.Vector3(.25,1,.15),scene); hemi.intensity=.95;
  const sun=new BABYLON.DirectionalLight('sun',new BABYLON.Vector3(-.4,-1,.3),scene); sun.position=new BABYLON.Vector3(80,140,-80); sun.intensity=.6;
  ...
  const mats={ground:mat('groundMat',new BABYLON.Color3(.56,.49,.36)),road:mat('roadMat',new BABYLON.Color3(.19,.22,.23)),concrete:mat('concreteMat',new BABYLON.Color3(.58,.61,.61)),amber:mat('amberMat',new BABYLON.Color3(1,.66,.22)),blue:mat('blueMat',new BABYLON.Color3(.05,.18,.25)),green:mat('greenMat',new BABYLON.Color3(.28,.75,.54)),white:mat('whiteMat',new BABYLON.Color3(.92,.93,.91)),dark:mat('darkMat',new BABYLON.Color3(.08,.09,.1))};
  ...
  function cyl(name,pos,diameter,height,material,parent=null,rotZ=0){const m=BABYLON.MeshBuilder.CreateCylinder(name,{diameter,height,tessellation:20},scene);m.position.copyFrom(pos);m.material=material;if(parent)m.parent=parent;m.rotation.z=rotZ;return m;}
  function road(x,z,w,d){return box('road',new BABYLON.Vector3(x,.08,z),new BABYLON.Vector3(w,.16,d),mats.road);}
  road(0,0,28,430);road(0,0,430,28);road(-150,-110,28,210);road(145,110,28,210);road(-75,-170,170,28);road(75,170,170,28);
  function building(x,z,w,h,d,material=mats.concrete){return box('building',new BABYLON.Vector3(x,h/2,z),new BABYLON.Vector3(w,h,d),material);}
  ...
  building(-174,-184,32,16,30,mats.blue);box('batchTower',new BABYLON.Vector3(-147,17,-184),new BABYLON.Vector3(13,34,13),mats.concrete);cyl('silo1',new BABYLON.Vector3(-165,20,-160),12,36,mats.white);cyl('silo2',new BABYLON.Vector3(-149,20,-160),12,36,mats.white);box('hopper',new BABYLON.Vector3(-147,7,-184),new BABYLON.Vector3(17,2,15),mats.amber);
  const destinations=[
  ...
  makeZone('depotZone',depot.x,depot.z,depot.r,mats.amber);const siteZones=destinations.map(d=>makeZone(`zone_${d.id}`,d.x,d.z,d.r,mats.green));siteZones.forEach(z=>z.setEnabled(false));

  const truck=new BABYLON.TransformNode('truck',scene);box('chassis',new BABYLON.Vector3(0,1.1,0),new BABYLON.Vector3(3.1,.7,7.2),mats.dark,truck);box('cab',new BABYLON.Vector3(0,2.25,2.25),new BABYLON.Vector3(3,2.7,2.5),mats.white,truck);box('windshield',new BABYLON.Vector3(0,2.6,3.52),new BABYLON.Vector3(2.45,1.15,.08),mats.blue,truck);
  const drum=cyl('drum',new BABYLON.Vector3(0,2.25,-.85),2.7,4.4,mats.white,truck,Math.PI/2);drum.rotation.x=Math.PI/2;drum.rotation.z=Math.PI/2;const stripe=cyl('stripe',new BABYLON.Vector3(0,2.25,-.85),2.78,.62,mats.amber,truck,Math.PI/2);stripe.rotation.x=Math.PI/2;stripe.rotation.z=Math.PI/2;
  [[-1.6,.8,2.1],[1.6,.8,2.1],[-1.6,.8,-1.7],[1.6,.8,-1.7],[-1.6,.8,-2.7],[1.6,.8,-2.7]].forEach((p,i)=>{const w=cyl(`wheel${i}`,new BABYLON.Vector3(...p),1.05,.55,mats.dark,truck,Math.PI/2);w.rotation.z=Math.PI/2;});
  ...
  function resetTruck(){truck.position.set(-150,.1,-202);truck.rotation.set(0,0,0);state.speed=0;state.steer=0;}
  ...
  function distanceTo(x,z){return Math.hypot(truck.position.x-x,truck.position.z-z);}function nearDepot(){return distanceTo(depot.x,depot.z)<=depot.r;}function nearSite(){return state.mission&&distanceTo(state.mission.x,state.mission.z)<=state.mission.r;}function kmh(){return Math.abs(state.speed)*5.4;}
  ...
  async function saveGame(reason='manual'){
    ...
    try{await saveInFlight;ui.saveInfo.textContent=`ذخیره شد · ${new Date(snapshot.timestamp).toLocaleTimeString('fa-IR')}`;return true;}catch(err){console.error('Save failed',err);ui.pauseStatus.textContent='ذخیره بازی ناموفق بود.';return false;}finally{saveInFlight=null;}
  }
  ...
  async function doAction(){if(!actionAvailable())return;if(state.phase==='AT_DEPOT'){state.phase='LOADING';state.actionHold=0;state.speed=0;dispatch(`سفارش ${state.mission.volume} مترمکعب ${state.mission.type}. بارگیری شروع شد.`);await saveGame('loading-start');}else if(state.phase==='TO_SITE'){state.phase='DELIVERING';state.actionHold=0;state.speed=0;await saveGame('delivery-start');}else if(state.phase==='RETURNING'){state.deliveries++;state.missionIndex=(state.missionIndex+1)%destinations.length;selectMission();state.phase='AT_DEPOT';dispatch('برگشتی کارخانه. سفارش بعدی آماده است.');await saveGame('returned-to-depot');}}
  ...
  let collisionCooldown=0;function collisionPenalty(){if(collisionCooldown>0)return;collisionCooldown=1.1;state.health=Math.max(0,state.health-7);state.score=Math.max(0,state.score-80);state.collisions++;state.speed*=-.18;dispatch('برخورد! سلامت خودرو و امتیاز کم شد.');if(state.health<=0)endShift('کامیون از سرویس خارج شد');}
  function truckPhysics(dt){const accel=input.up?7.2:0,reverse=input.down?5.2:0,drag=1.8;if(accel)state.speed+=accel*dt;if(reverse)state.speed-=reverse*dt;if(!input.up&&!input.down)state.speed-=Math.sign(state.speed)*Math.min(Math.abs(state.speed),drag*dt);if(input.brake)state.speed*=Math.max(0,1-5*dt);state.speed=Math.max(-5,Math.min(13,state.speed));const steerTarget=(input.left?1:0)+(input.right?-1:0);state.steer+=(steerTarget-state.steer)*Math.min(1,5*dt);if(Math.abs(state.speed)>.15)truck.rotation.y+=state.steer*(.38+.028*Math.abs(state.speed))*dt*Math.sign(state.speed);const fwd=new BABYLON.Vector3(Math.sin(truck.rotation.y),0,Math.cos(truck.rotation.y));truck.position.addInPlace(fwd.scale(state.speed*dt));drum.rotation.y+=(state.loaded?1.8:.5)*dt;stripe.rotation.y=drum.rotation.y;clampWorld();}
  function cameraUpdate(dt){const fwd=new BABYLON.Vector3(Math.sin(truck.rotation.y),0,Math.cos(truck.rotation.y));const right=new BABYLON.Vector3(fwd.z,0,-fwd.x);let desired,target;if(state.cameraMode===0){desired=truck.position.subtract(fwd.scale(13)).add(new BABYLON.Vector3(0,7.5,0));target=truck.position.add(fwd.scale(7)).add(new BABYLON.Vector3(0,2.1,0));}else{desired=truck.position.add(fwd.scale(2.7)).add(right.scale(-.35)).add(new BABYLON.Vector3(0,3.2,0));target=truck.position.add(fwd.scale(24)).add(new BABYLON.Vector3(0,2.6,0));}camera.position=BABYLON.Vector3.Lerp(camera.position,desired,Math.min(1,5*dt));camera.setTarget(target);}
  function updateMission(dt){if(!state.running||paused)return;state.time-=dt;if(state.loaded)state.quality=Math.max(0,state.quality-dt*.11-(kmh()>65?dt*.025:0));if(state.time<=0){endShift('زمان مأموریت تمام شد');return;}if(state.quality<=18){endShift('کیفیت بتن به حد مردودی رسید');return;}if(state.phase==='AT_DEPOT'){setMissionUI('مرحله ۱ از ۴','بارگیری در کارخانه',`زیر بچینگ توقف کن. سفارش: ${state.mission.volume} m³ ${state.mission.type}`,0);}else if(state.phase==='LOADING'){state.actionHold+=dt;const p=Math.min(100,state.actionHold/3.2*100);setMissionUI('در حال بارگیری','بچینگ در حال بارگیری است','حرکت نکن؛ بارگیری تا تکمیل ادامه دارد.',p);if(p>=100)completeLoading();}else if(state.phase==='TO_SITE'){const dist=Math.round(distanceTo(state.mission.x,state.mission.z));setMissionUI('مرحله ۲ از ۴',`حرکت به ${state.mission.name}`,`فاصله تقریبی ${dist} متر · بتن را با کیفیت مناسب برسان.`,Math.max(0,100-dist/3));if(state.time<45&&performance.now()/1000-state.lastDispatch>12){state.lastDispatch=performance.now()/1000;dispatch('راننده! کمتر از ۴۵ ثانیه وقت داری؛ معطل نکن!');}}else if(state.phase==='DELIVERING'){state.actionHold+=dt;const p=Math.min(100,state.actionHold/3.6*100);setMissionUI('مرحله ۳ از ۴','تخلیه بتن','پارک ثبت شد؛ در حال تخلیه.',p);if(p>=100)completeDelivery();}else if(state.phase==='RETURNING'){const dist=Math.round(distanceTo(depot.x,depot.z));setMissionUI('مرحله ۴ از ۴','بازگشت به کارخانه',`برای سفارش بعدی برگرد. فاصله ${dist} متر.`,Math.max(0,100-dist/3));}}
  function updateHUD(){ui.timer.textContent=`${String(Math.max(0,Math.floor(state.time/60))).padStart(2,'0')}:${String(Math.max(0,Math.floor(state.time%60))).padStart(2,'0')}`;ui.quality.textContent=`${Math.round(state.quality)}%`;ui.health.textContent=`${Math.round(state.health)}%`;ui.score.textContent=state.score.toLocaleString('fa-IR');ui.speed.textContent=Math.round(kmh());const available=actionAvailable();ui.actionBtn.disabled=!available;ui.actionBtn.style.opacity=available?'1':'.5';if(state.phase==='AT_DEPOT')ui.actionHint.textContent=available?'برای شروع بارگیری E را بزن':'وارد محدوده بچینگ شو و توقف کن';else if(state.phase==='TO_SITE')ui.actionHint.textContent=available?'پارک انجام شد؛ E برای تخلیه':'داخل حلقه سبز پروژه توقف کن';else if(state.phase==='RETURNING')ui.actionHint.textContent=available?'E برای سفارش بعدی':'به کارخانه برگرد';else ui.actionHint.textContent='عملیات در حال انجام است…';}
